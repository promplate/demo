...
async def invoke(data: ChainInput, node: Node = Depends(get_node)):
    """Route handler for invoking a specific template.

    Args:
        data (ChainInput): The input data containing messages,
            model configuration and other parameters to pass to the template.
        node (Node): The template node that will process the input data.

    Returns:
        The response generated by processing the template with the provided data.
    """
    try:
